...
func CheckOpenAIConnectivity(config *DiagnosticConfig) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("OpenAI API responded with status %s: %s", resp.Status, string(bodyBytes))
	}
...
func main() {
	...
	config := &DiagnosticConfig{
		...
		OpenAIEndpoint:   os.Getenv("OPENAI_ENDPOINT"), // Ensure this env var is set
	...
	if err := CheckEnvironmentVariables(config); err != nil {
		log.Fatalf("environment variables check failed: %v", err)
	}
	...
	if err := CheckS3Connectivity(config); err != nil {
		log.Fatalf("s3 connectivity check failed: %v", err)
	}
	...
	if err := CheckOpenAIConnectivity(config); err != nil {
		log.Fatalf("openai api connectivity check failed: %v", err)
	}
	...
	if err := CheckTelegramConnectivity(config); err != nil {
		log.Fatalf("telegram api connectivity check failed: %v", err)
	}
	...
	if err := CheckPortAvailability(config.Port); err != nil {
		log.Fatalf("port availability check failed: %v", err)
	}
	log.Println("Port Availability Check Passed.")

	log.Println("All Diagnostic Checks Passed Successfully.")
}
